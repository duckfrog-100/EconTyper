...
import {
  ...
  TypingSettings,
  type TypingSettingsValue,
} from "@/components/practice/typing-settings";
import {
...
function loadSettings(): TypingSettingsValue {
  try {
    ...
    return { ...DEFAULT_TYPING_SETTINGS, ...(JSON.parse(raw) as Partial<TypingSettingsValue>) };
  } catch {
...
export default function PracticePage({ searchParams }: PracticePageProps) {
  ...
  useEffect(() => {
    try {
      ...
      // Settings remain available for this render even when storage is unavailable.
    }
  }, [settings]);
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-5 py-8 sm:px-8 sm:py-12">
      <div className="flex items-center justify-between gap-4">
        ...
        <button type="button" onClick={() => setSettingsOpen(true)} className="rounded-xl border border-zinc-200 px-4 py-2 text-xs font-semibold uppercase tracking-wider hover:bg-zinc-100 dark:border-zinc-800 dark:hover:bg-zinc-900">Typing Settings ⚙</button>
      </div>
      ...
      <section className="mt-7 rounded-3xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900 sm:p-8">
        <p className="mb-5 text-xs font-semibold uppercase tracking-widest text-zinc-500">Read · hover for Korean meaning · double-click to copy</p>
        <div className="space-y-7">
          {settings.contextMode === "three" && sentenceIndex > 0 && (
            <InteractiveSentence sentence={sentences[sentenceIndex - 1].text} muted style={textStyle} />
          )}
          <InteractiveSentence sentence={sentence} style={textStyle} />
          {settings.contextMode === "three" && sentenceIndex < sentences.length - 1 && (
            <InteractiveSentence sentence={sentences[sentenceIndex + 1].text} muted style={textStyle} />
          )}
        </div>
      </section>
      ...
      <section className="mt-6">
        ...
        <div
          ...
          className="relative min-h-48 cursor-text rounded-3xl border border-zinc-300 bg-white p-6 shadow-sm outline-none focus-within:border-emerald-500 focus-within:ring-4 focus-within:ring-emerald-500/10 dark:border-zinc-700 dark:bg-zinc-900 sm:p-8"
        >
